...
const USER_COLUMNS = 'id, name, email, base_currency AS "baseCurrency", created_at AS "createdAt"';
...
export const userModel = {
  ...
  async findByEmail(email) {
    const { rows } = await query(
      `SELECT id, name, email, password_hash, base_currency AS "baseCurrency", created_at AS "createdAt" FROM users WHERE email = $1`,
      [email]
    );
    return rows[0] || null;
  },
